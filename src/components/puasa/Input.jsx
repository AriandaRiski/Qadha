...
const Input = ({puasa, handleDelete, index, setEditPuasa}) => {
  ...
  const fetchId = async(id) => {
    ...
    return (
      <>
          <tr>
            <td>{index+1}</td>
            <td>{puasa.nama_puasa}</td>
            <td>{moment(puasa.tanggal_puasa).format('DD MMMM YYYY')}</td>
            <td>
              ...
              <Button variant="warning" href="#editModal" className="edit" data-toggle="modal" onClick={() => fetchId(puasa.id)}><AiFillEdit /></Button>
          ...
          <Modal show={show} onHide={handleClose} animation={true}>
            ...
            <Modal.Footer>
              ...
              <Button size="sm" variant="danger" onClick={() => handleDelete(puasa.id)} onClose={handleClose}>Ya</Button>
            </Modal.Footer>
          </Modal>
      </>
    )
